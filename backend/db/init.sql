...
-- =========================================================
-- Mejoras: estado + auditoría + constraints coherentes + updates automáticos activados por trigger
-- Para usar gen_random_uuid si a futuro se migran los IDs a UUID (para usar con microservicios):
-- CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- DROP en orden por dependencias
DROP TRIGGER IF EXISTS trg_validar_convivencia_voto ON voto_usuario;
DROP FUNCTION IF EXISTS validar_convivencia_voto();

DROP TRIGGER IF EXISTS trg_voto_usuario_updated_at ON voto_usuario;
DROP TRIGGER IF EXISTS trg_usuario_habitacion_updated_at ON usuario_habitacion;
DROP TRIGGER IF EXISTS trg_habitacion_updated_at ON habitacion;
DROP TRIGGER IF EXISTS trg_piso_updated_at ON piso;
...
CREATE TABLE usuario (
  ...
  apellidos VARCHAR(150) NOT NULL CHECK (BTRIM(apellidos) <> ''),
  email VARCHAR(150) NOT NULL CHECK (BTRIM(email) <> ''),
  password_hash VARCHAR(255) NOT NULL CHECK (BTRIM(password_hash) <> ''),
  rol VARCHAR(20) NOT NULL DEFAULT 'user' CHECK (rol IN ('user', 'advertiser', 'admin')),
  telefono VARCHAR(20),
...
-- 1) LOGIN / recuperación / cualquier lookup por email (case-insensitive)
CREATE UNIQUE INDEX uq_usuario_email_lower ON usuario (LOWER(email));
-- 2) Panel admin: listar usuarios por estado + rol (muy típico)
CREATE INDEX idx_usuario_activo_rol ON usuario (activo, rol);
-- 3) Panel admin: listados paginados/ordenados por fecha de alta (útil para "últimos registrados", etc.)
CREATE INDEX idx_usuario_fecha_registro_desc ON usuario (fecha_registro DESC);
-- 4) Panel admin: ordenaciones por updated_at (auditoría / últimas modificaciones)
CREATE INDEX idx_usuario_updated_at_desc ON usuario (updated_at DESC);
-- 5) Búsqueda por nombre/apellidos (sin FTS): soporta prefix búsquedas tipo "garc%" Ojo: para búsquedas “contiene” (%arc%) no sirve; para eso es FTS.
CREATE INDEX idx_usuario_apellidos_lower ON usuario (LOWER(apellidos));
CREATE INDEX idx_usuario_nombre_lower ON usuario (LOWER(nombre));
-- 6) Búsqueda rápida por nombre completo (útil para admin)
CREATE INDEX idx_usuario_nombre_completo_lowerON usuario ((LOWER(nombre) || ' ' || LOWER(apellidos)));
-- 7) (Opcional) Permitir lookup por teléfono (soporte / admin)
...
CREATE TABLE piso (
  ...
  direccion VARCHAR(255) NOT NULL CHECK (BTRIM(direccion) <> ''),
  ciudad VARCHAR(100) NOT NULL CHECK (BTRIM(ciudad) <> ''),
...
-- Filtro principal por ciudad (búsquedas frontend)
CREATE INDEX idx_piso_ciudad ON piso (ciudad);
-- Listado público: pisos por ciudad y estado activo
CREATE INDEX idx_piso_ciudad_activo ON piso (ciudad, activo);
-- Optimiza búsquedas solo de pisos activos por ciudad
CREATE INDEX idx_piso_ciudad_activos_only ON piso (ciudad) WHERE activo = true;
-- Panel advertiser/admin: listar pisos de un manager
CREATE INDEX idx_piso_manager_usuario ON piso (manager_usuario_id);
-- Gestión anuncios: pisos de un manager filtrados por activo
CREATE INDEX idx_piso_manager_activo ON piso (manager_usuario_id, activo);
-- Ordenación por fecha de creación (últimos publicados)
CREATE INDEX idx_piso_created_at_desc ON piso (created_at DESC);
-- Ordenación por última modificación (auditoría/backoffice)
CREATE INDEX idx_piso_updated_at_desc ON piso (updated_at DESC);


CREATE TRIGGER trg_piso_updated_at
...
CREATE TABLE habitacion (
  ...
  activo     BOOLEAN NOT NULL DEFAULT true,
  tamano_m2 INT CHECK (tamano_m2 IS NULL OR tamano_m2 > 0),
...
-- Join con piso para búsquedas por ciudad o filtros combinados
CREATE INDEX idx_habitacion_piso_id ON habitacion (piso_id);
-- Listado público principal: habitaciones visibles ordenables por precio
CREATE INDEX idx_habitacion_visible_precio ON habitacion (precio_mensual) WHERE activo = true AND disponible = true;
-- Filtro general por estado activo
CREATE INDEX idx_habitacion_activo ON habitacion (activo);
-- Filtro general por disponibilidad
CREATE INDEX idx_habitacion_disponible ON habitacion (disponible);
-- Filtro por habitaciones amuebladas
CREATE INDEX idx_habitacion_amueblada ON habitacion (amueblada);
-- Filtro por baño privado
CREATE INDEX idx_habitacion_bano ON habitacion (bano);
-- Filtro por balcón
CREATE INDEX idx_habitacion_balcon ON habitacion (balcon);
-- Filtro por tamaño de habitación
CREATE INDEX idx_habitacion_tamano ON habitacion (tamano_m2);
-- Ordenación y filtros combinados de visibles por precio y tamaño
CREATE INDEX idx_habitacion_visible_precio_tamano ON habitacion (precio_mensual, tamano_m2) WHERE activo = true AND disponible = true;
-- Filtro de visibles con baño privado (optimiza checkbox “baño privado”)
CREATE INDEX idx_habitacion_visible_bano ON habitacion (precio_mensual) WHERE activo = true AND disponible = true AND bano = true;
-- Filtro de visibles con balcón (optimiza checkbox “balcón”)
CREATE INDEX idx_habitacion_visible_balcon ON habitacion (precio_mensual) WHERE activo = true AND disponible = true AND balcon = true;
-- Buscador de texto libre en título y descripción (Full-Text Search)
CREATE INDEX idx_habitacion_fts ON habitacion USING GIN (to_tsvector('spanish', coalesce(titulo,'') || ' ' || coalesce(descripcion,'')));
...
-- Join para cargar todas las fotos de un piso
CREATE INDEX idx_foto_piso_piso_id ON foto_piso (piso_id);
-- Ordenar fotos de un piso en carrusel
CREATE INDEX idx_foto_piso_orden ON foto_piso (piso_id, orden);
-- Evita duplicar posiciones de carrusel dentro de un mismo piso
...
-- Join para cargar todas las fotos de una habitación
CREATE INDEX idx_foto_habitacion_habitacion_id ON foto_habitacion (habitacion_id);
-- Ordenar fotos de una habitación en carrusel
CREATE INDEX idx_foto_habitacion_orden ON foto_habitacion (habitacion_id, orden);
-- Evita duplicar posiciones de carrusel dentro de una misma habitación
...
CREATE TABLE usuario_habitacion (
  ...
  estado VARCHAR(20) NOT NULL DEFAULT 'active'
    CHECK (estado IN ('active', 'left', 'kicked')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  
  CHECK (fecha_salida IS NULL OR fecha_salida >= fecha_entrada),
  -- coherencia: activa => fecha_salida NULL; cerrada => fecha_salida NOT NULL
  CHECK (
    (fecha_salida IS NULL AND estado = 'active')
    OR
    (fecha_salida IS NOT NULL AND estado IN ('left','kicked'))
  )
);

-- Un usuario no puede tener 2 estancias activas
CREATE UNIQUE INDEX uq_usuario_estancia_activa ON usuario_habitacion(usuario_id) WHERE fecha_salida IS NULL;
-- Una habitación no puede tener 2 ocupaciones activas
CREATE UNIQUE INDEX uq_habitacion_ocupacion_activa ON usuario_habitacion(habitacion_id) WHERE fecha_salida IS NULL;
-- Join para obtener todas las estancias de un usuario
CREATE INDEX idx_usuario_habitacion_usuario_id ON usuario_habitacion (usuario_id);
-- Join para obtener ocupantes de una habitación
CREATE INDEX idx_usuario_habitacion_habitacion_id ON usuario_habitacion (habitacion_id);
-- Consultas por estancias activas de un usuario
CREATE INDEX idx_usuario_habitacion_usuario_activa ON usuario_habitacion (usuario_id) WHERE fecha_salida IS NULL;
-- Consultas por ocupación activa de una habitación
CREATE INDEX idx_usuario_habitacion_habitacion_activa ON usuario_habitacion (habitacion_id) WHERE fecha_salida IS NULL;
-- Validaciones y consultas por fechas de convivencia de un usuario
CREATE INDEX idx_usuario_habitacion_usuario_fechas ON usuario_habitacion (usuario_id, fecha_entrada, fecha_salida);
-- Validaciones y consultas por fechas dentro de una habitación
CREATE INDEX idx_usuario_habitacion_habitacion_fechas ON usuario_habitacion (habitacion_id, fecha_entrada, fecha_salida);
-- Filtro por estado de estancia (active / left / kicked)
CREATE INDEX idx_usuario_habitacion_estado ON usuario_habitacion (estado);
-- Histórico: consultas de estancias cerradas
CREATE INDEX idx_usuario_habitacion_cerradas ON usuario_habitacion (usuario_id, fecha_salida) WHERE fecha_salida IS NOT NULL;

CREATE TRIGGER trg_usuario_habitacion_updated_at
BEFORE UPDATE ON usuario_habitacion
FOR EACH ROW
EXECUTE FUNCTION set_updated_at();
...
CREATE TABLE voto_usuario (
  ...
  num_cambios INT NOT NULL DEFAULT 0 CHECK (num_cambios >= 0),
  created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
-- Garantiza un solo voto por (piso, votante, votado)
CREATE UNIQUE INDEX uq_voto_usuario_piso_votante_votado ON voto_usuario (piso_id, votante_id, votado_id);
-- Listar votos recibidos por un usuario (reputación)
CREATE INDEX idx_voto_usuario_votado_id ON voto_usuario (votado_id);
-- Listar votos emitidos por un usuario (histórico de votaciones)
CREATE INDEX idx_voto_usuario_votante_id ON voto_usuario (votante_id);
-- Filtrar/listar votos por piso (dashboard del piso)
CREATE INDEX idx_voto_usuario_piso_id ON voto_usuario (piso_id);
-- Ordenar/filtrar por fecha de voto (últimos votos)
CREATE INDEX idx_voto_usuario_created_at_desc ON voto_usuario (created_at DESC);
-- Consultas típicas: reputación por usuario ordenada por fecha
CREATE INDEX idx_voto_usuario_votado_fecha ON voto_usuario (votado_id, created_at DESC);
-- Consultas típicas: actividad de un votante ordenada por fecha
CREATE INDEX idx_voto_usuario_votante_fecha ON voto_usuario (votante_id, created_at DESC);

CREATE TRIGGER trg_voto_usuario_updated_at
BEFORE UPDATE ON voto_usuario
FOR EACH ROW
EXECUTE FUNCTION set_updated_at();
...
-- =========================================================
INSERT INTO usuario (nombre, apellidos, email, password_hash, rol, telefono, foto_perfil_url, activo, fecha_registro) VALUES
...
-- =========================================================
INSERT INTO piso (direccion, ciudad, codigo_postal, descripcion, manager_usuario_id, activo, created_at) VALUES
...
-- 3 por piso (7 pisos)
-- =========================================================
INSERT INTO habitacion (piso_id, titulo, descripcion, precio_mensual, disponible, tamano_m2, amueblada, bano, balcon, created_at) VALUES
(1,'Exterior con balcón (luz todo el día)','Habitación exterior con balcón a calle tranquila. Cama 135, armario grande y escritorio. Muy buena para teletrabajo.',540,true,14,true,false,true,'2026-01-07 10:30:00+01'),
(1,'Interior silenciosa (ideal estudio)','Habitación interior muy silenciosa. Cama 90, escritorio y estanterías. Perfecta si valoras el descanso.',430,true,10,true,false,false,'2026-01-07 10:35:00+01'),
(1,'Grande con zona de trabajo','Cama 150, escritorio amplio y silla cómoda. Ventana a patio amplio (entra luz sin ruido).',510,false,15,true,true,false,'2026-01-07 10:40:00+01'),
(2,'Techos altos en Eixample','Habitación con techos altos, cama 135 y armario empotrado. Buen aislamiento. Ambiente tranquilo.',650,true,15,true,true,true,'2026-01-07 13:00:00+01'),
(2,'Individual cerca del metro','Cama 90, escritorio y almacenamiento. Buena relación calidad/precio para la zona.',520,true,9,true,false,false,'2026-01-07 13:05:00+01'),
(2,'Amplia con luz natural','Cama 135, armario grande y ventilador de techo. Ventana grande y buena ventilación.',610,false,14,true,true,true,'2026-01-07 13:10:00+01'),
(3,'Ventana a calle tranquila (Ruzafa)','Cama 135, escritorio y armario. Calle poco transitada por la noche. Zonas comunes cuidadas.',440,true,12,true,false,false,'2026-01-08 09:20:00+01'),
(3,'Compacta y económica','Cama 90 y escritorio plegable. Ideal si buscas ahorrar. (Sin muebles extra, se puede traer lo propio).',330,true,8,false,false,false,'2026-01-08 09:25:00+01'),
(3,'Balcón francés y mucha luz','Muy luminosa, cama 135 y escritorio grande. Buen armario y espacio para almacenaje.',480,true,13,true,false,true,'2026-01-08 09:30:00+01'),
(4,'A/C individual + cama 135','Aire acondicionado propio. Cama 135, armario y escritorio. Buena para verano.',400,true,12,true,false,false,'2026-01-08 18:40:00+01'),
(4,'Interior silenciosa (descanso)','Cama 90, armario alto. Muy buena para descansar. Ventilación correcta.',320,true,9,true,false,false,'2026-01-08 18:45:00+01'),
(4,'Exterior luminosa (ventana grande)','Cama 150, escritorio y estantería. Ventana grande y buena luz natural.',430,false,14,true,true,true,'2026-01-08 18:50:00+01'),
(5,'Teletrabajo: escritorio grande','Escritorio grande, silla cómoda y buena luz. Cama 135. Calefacción central incluida en comunidad.',590,true,14,true,true,false,'2026-01-09 11:25:00+01'),
(5,'Individual económica (Indautxu)','Cama 90, armario, escritorio. Muy buena ubicación, ambiente serio.',480,true,10,true,false,false,'2026-01-09 11:30:00+01'),
(5,'Doble con armario empotrado','Cama 150, armario empotrado y espacio extra. Habitación más “premium” del piso.',650,true,15,true,true,false,'2026-01-09 11:35:00+01'),
(6,'Mucha luz + cama 135','Cama 135, escritorio y ventilador. Muy luminosa. Buen aislamiento acústico.',490,true,12,true,false,true,'2026-01-09 18:00:00+01'),
(6,'Tranquila y fresca (sin amueblar)','Habitación fresca con ventilación cruzada. Sin muebles (ideal si ya tienes).',380,true,10,false,false,false,'2026-01-09 18:05:00+01'),
(6,'Grande con escritorio amplio','Cama 150, escritorio grande y estanterías. Perfecta para teletrabajo.',540,false,14,true,true,true,'2026-01-09 18:10:00+01'),
(7,'Exterior con vistas (piso en reforma)','Habitación exterior. El piso está temporalmente inactivo por reforma de baño; se retomarán visitas en breve.',420,true,12,true,false,true,'2026-01-05 16:40:00+01'),
(7,'Interior tranquila (piso inactivo)','Habitación interior tranquila. Ideal para estudiante. Se reactivará el anuncio al finalizar la obra.',350,true,9,true,false,false,'2026-01-05 16:45:00+01'),
(7,'Amplia con armario grande (piso inactivo)','Cama 135 y armario grande. Buena luz. Aún no se hacen visitas por reforma.',460,true,13,true,true,false,'2026-01-05 16:50:00+01');

-- =========================================================
-- INSERTS: foto_piso (3 por piso, 7 pisos)
-- orden: 0 portada, 1,2 siguientes
-- =========================================================
INSERT INTO foto_piso (piso_id, url, orden, created_at) VALUES
...
-- =========================================================
INSERT INTO usuario_habitacion (usuario_id, habitacion_id, fecha_entrada, fecha_salida, estado) VALUES
-- ESTANCIAS ACTIVAS (usuarios 7..15)
-- Piso 1 (habitaciones 1-3): 7,8,9 conviven ahora
(7,  1, '2026-01-10 19:00:00+01', NULL, 'active'),
(8,  2, '2026-01-11 09:00:00+01', NULL, 'active'),
(9,  3, '2026-01-11 18:30:00+01', NULL, 'active'),
-- Piso 2 (habitaciones 4-6): 10,11,12 conviven ahora
(10, 4, '2026-01-10 20:00:00+01', NULL, 'active'),
(11, 5, '2026-01-11 12:00:00+01', NULL, 'active'),
(12, 6, '2026-01-11 21:00:00+01', NULL, 'active'),
-- Piso 3 (habitaciones 7-9): 13,14,15 conviven ahora
(13, 7, '2026-01-09 18:00:00+01', NULL, 'active'),
(14, 8, '2026-01-10 10:30:00+01', NULL, 'active'),
(15, 9, '2026-01-10 13:15:00+01', NULL, 'active'),
...
(16, 10, '2025-09-05 12:00:00+02', '2025-12-10 10:00:00+01', 'left'),
(17, 11, '2025-09-20 12:00:00+02', '2025-12-22 12:00:00+01', 'left'),
(18, 12, '2025-10-10 12:00:00+02', '2026-01-05 09:00:00+01', 'left'),
-- Piso 5 (habitaciones 13-15): 19 vivió un tiempo (solo 1 histórico aquí, útil para edge cases)
(19, 13, '2025-06-01 10:00:00+02', '2025-11-15 10:00:00+01', 'left'),
-- Piso 6 (habitaciones 16-18): 20 vivió un tiempo
(20, 16, '2025-07-10 11:00:00+02', '2025-12-01 10:00:00+01', 'left');
...
-------------------------------------------------------
INSERT INTO voto_usuario (piso_id, votante_id, votado_id, limpieza, ruido, puntualidad_pagos, num_cambios, created_at) VALUES
-- Piso 1 (usuarios 7,8,9) - convivencia activa
...
-- Piso 2 (usuarios 10,11,12) - convivencia activa
...
-- Piso 3 (usuarios 13,14,15) - convivencia activa
...
-- Piso 4 (histórico: usuarios 16,17,18) - convivencia pasada con solape real
